...
export default function Register() {
  ...
  const handleRegistrationCheck = async (e: FormEvent) => {
    try {
      e.preventDefault()
      setLoading(true);
      setError('Registrace nebyla dokončena kvůli internímu problém aplikace');
      setLoading(false);
    } catch (error: any) {
      setLoading(false);
      setError(error)
      ...
      >
        ...
        >
          ...
          <p className={'text-center'}>
            Na e-mail Vám zašleme potvrzovací zprávu,
            poté budete moct pokračovat ve vytváření profilu
            vaší školy.
          </p>
          <div className={"mb-4 mt-auto flex flex-col gap-2"}>
            ...
            <button disabled={loading || canSubmit} type="submit" className={`disabled:opacity-50 bg-[#0066ff] border-[0] w-full rounded text-white text-center py-3 px-3`}>
